<?php
   ...
   if (!isset($_POST['cogni_fg']) || !isset($_POST['pface_id']) || !isset($_POST['user_data'])) {// parameters missing
        ...
        $n_rows = 0;
        if ($stmt = $link->prepare("UPDATE persisted_face_data SET user_data = ? WHERE cogni_fg = ? AND pface_id = ?")) {
            $stmt->bind_param("sss", $_POST['user_data'], $_POST['cogni_fg'], $_POST['pface_id']);
        ...
        if($n_rows > 0){
            ...
            echo ('Patched face correctly for face group '.$_POST['cogni_fg'].' on cogni storage');
        }else{
            http_response_code(400);
            echo ('Bad request');
        }
